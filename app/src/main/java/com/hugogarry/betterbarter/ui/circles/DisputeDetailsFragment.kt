...
class DisputeDetailsFragment : Fragment() {
    ...
    // private val args: AdminDisputeDetailsFragmentArgs by navArgs()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Assume we passed the dispute ID, for now hardcoding a fetch
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.disputeDetails.collectLatest { resource ->
                if (resource is Resource.Success) {
                    ...
                    textReason.text = currentDispute?.reason
                    radioInitiator.text = "Initiator: ${currentDispute?.initiator?.username}"
                    radioRespondent.text = "Respondent: ${currentDispute?.respondent?.username}"
                }
            }
        }

        btnGoToChat.setOnClickListener {
            currentDispute?.chatId?.let { chatId ->
                // Assuming Nav action is setup
                // findNavController().navigate(AdminDisputeDetailsFragmentDirections.actionAdminDisputeDetailsToChat(chatId))
            }
        }

        btnResolve.setOnClickListener {
            val culpritId = if (radioInitiator.isChecked) currentDispute?.initiator?.id else currentDispute?.respondent?.id
